...
export function processHeading(line, config, style, documentType) {
    ...
    const processedTextRuns = processFormattedTextForHeading(headingText, headingSize);
 ...
 */
function processFormattedTextForHeading(text, fontSize) {
    const textRuns = [];
    let currentText = "";
    let isBold = false;
    let isItalic = false;
...
function calculateImageSize(blob) {
...
export function processParagraph(text, style) {
    ...
    console.log(`Paragraph alignment: ${alignment}, Style alignment: ${style.paragraphAlignment}`);
