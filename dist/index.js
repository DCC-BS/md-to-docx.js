import { Document, Paragraph, TextRun, AlignmentType, PageOrientation, Packer, PageBreak, InternalHyperlink, Footer, PageNumber, LevelFormat, } from "docx";
import saveAs from "file-saver";
import { headingConfigs } from "./types";
import { processHeading, processTable, processListItem, processBlockquote, processComment, collectTables, processCodeBlock, processLinkParagraph, processImage, processParagraph, } from "./helpers";
const defaultStyle = {
    titleSize: 32,
    headingSpacing: 240,
 ...
 */
export async function convertMarkdownToDocx(markdown, options = defaultOptions) {
    try {
            ...
            try {
                ...
                if (trimmedLine.startsWith("```")) {
                    if (!inCodeBlock) {
                        ...
                        // End of code block
                        inCodeBlock = false;
                        docChildren.push(processCodeBlock(codeBlockContent.trim(), codeBlockLanguage, style));
                        codeBlockContent = "";
                    ...
                    if (match) {
                        ...
                        if (level >= 1 && level <= 5) {
                            ...
                            const config = {
                                ...
                                alignment: headingConfigs[level].alignment || style.headingAlignment,
                            };
                            const { paragraph: headingParagraph, bookmarkId } = processHeading(trimmedLine, config, style, documentType);
                ...
                if (trimmedLine.startsWith("|") && trimmedLine.endsWith("|")) {
                    if (i + 1 < lines.length &&
                        ...
                        if (tableIndex < tables.length) {
                            try {
                                docChildren.push(processTable(tables[tableIndex], documentType));
                ...
                if (/^\s*\d+\.\s/.test(trimmedLine)) {
                    ...
                    listItems.push(processListItem({
                        text: listText,
                        boldText,
                        isNumbered: true,
                ...
                if (imageMatch) {
                    ...
                    try {
                        ...
                        const imageParagraphs = await processImage(altText, imageUrl, style);
                        console.log(`Successfully processed image, adding ${imageParagraphs.length} paragraphs`);
