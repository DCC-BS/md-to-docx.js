...
import { convertMarkdownToDocx, type Options } from "../src/index";
...
##### Heading 5`;
        ...
        const outputPath = path.join(
            outputDir,
            "default-heading-alignment.docx",
        );
...
##### Heading 5`;
        ...
        const outputPath = path.join(
            outputDir,
            "fallback-heading-alignment.docx",
        );
        const arrayBuffer = await buffer.arrayBuffer();
        fs.writeFileSync(outputPath, Buffer.from(arrayBuffer));
...
##### Heading 5 (should be centered)`;
        ...
        const outputPath = path.join(
            outputDir,
            "priority-heading-alignment.docx",
        );
        const arrayBuffer = await buffer.arrayBuffer();
        fs.writeFileSync(outputPath, Buffer.from(arrayBuffer));
